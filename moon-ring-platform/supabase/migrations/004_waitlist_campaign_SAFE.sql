-- Migration 004: Waitlist Campaign Infrastructure (SAFE VERSION)
-- This version uses IF NOT EXISTS and DROP IF EXISTS to avoid duplicate errors
...
-- Created: 2025-01-16
-- Updated: 2025-10-28 (added safe guards for reruns)

-- ============================================================================
-- PART 0: Cleanup any partial migrations (safe to run multiple times)
-- ============================================================================

-- Drop existing policies (if they exist)
DROP POLICY IF EXISTS "Users can view their own referrals" ON waitlist_referrals;
DROP POLICY IF EXISTS "Service role can manage referrals" ON waitlist_referrals;
DROP POLICY IF EXISTS "Users can view their own email events" ON email_campaign_events;
DROP POLICY IF EXISTS "Service role can manage email events" ON email_campaign_events;
DROP POLICY IF EXISTS "Users can view their own commitments" ON micro_commitments;
DROP POLICY IF EXISTS "Users can create their own commitments" ON micro_commitments;
DROP POLICY IF EXISTS "Service role can manage commitments" ON micro_commitments;

-- Drop existing triggers (if they exist)
DROP TRIGGER IF EXISTS trigger_generate_referral_code ON email_subscriptions;
DROP TRIGGER IF EXISTS trigger_update_engagement_score ON email_campaign_events;
DROP TRIGGER IF EXISTS trigger_update_positions ON email_subscriptions;
...
BEGIN
  ...
  -- Calculate score: opens (1pt), clicks (3pts), referrals (10pts), commitments (5pts)
...
-- Policies for waitlist_referrals
...
-- Policies for email_campaign_events
...
-- Policies for micro_commitments
...
-- ============================================================================
-- PART 8: Initialize data for existing subscribers
-- ============================================================================
...
BEGIN
  ...
  RAISE NOTICE 'Created tables: waitlist_referrals, email_campaign_events, micro_commitments';
  RAISE NOTICE 'Added % columns to email_subscriptions', (
    SELECT COUNT(*) FROM information_schema.columns
    WHERE table_name = 'email_subscriptions'
    AND column_name LIKE ANY(ARRAY['%waitlist%', '%referral%', '%campaign%', '%engagement%'])
  );
END $$;
